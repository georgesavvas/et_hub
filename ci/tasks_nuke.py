...
def test(ctx, coverage='', path='', tests=''):
    ...
    ctx.run(
        "rez env {0}-{1} python-2 nuke-12 volt_nuke_session_tvc pkg_resources pytest pytest_cov mock -- nuke -t runtests.py".format(
            NAME, version
        )
    )

    ctx.run(
        "rez env {0}-{1} python-3 nuke-13 volt_nuke_session_tvc pytest pytest_cov mock  -- nuke -t runtests.py".format(
            NAME, version
        )
    )
